...
const PurchasePaymentModal = ({ isOpen, onClose, total, onConfirm, selectedSupplier = null, loading = false }) => {
  ...
  const handleConfirm = () => {
    ...
    onConfirm(payments, receiveImmediately)
  }
      ...
      <div className="bg-white shadow-xl w-full max-w-6xl h-[90vh] flex flex-col overflow-hidden relative z-[101]">
        ...
        <div className="flex-shrink-0 flex items-center justify-between p-6 bg-slate-800">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-xl font-semibold text-white">Configurar Pago de Compra</h2>
              <p className="text-sm text-slate-300 mt-1">Configure los métodos de pago para la compra</p>
            </div>
          </div>
                ...
                <Card className="border-blue-200 bg-blue-50 shadow-sm">
                  ...
                  <CardContent className="p-4 space-y-4">
                    <div className="flex items-start space-x-3">
                      <Checkbox
                        id="receive-immediately"
                        checked={receiveImmediately}
                        onCheckedChange={setReceiveImmediately}
                        className="mt-1"
                        disabled={loading}
                      />
                      <div className="space-y-2">
                        <Label
                          htmlFor="receive-immediately"
                          className="text-sm font-medium text-blue-900 cursor-pointer"
                        >
                          Recibir productos inmediatamente
                        </Label>
                        <p className="text-xs text-blue-700 leading-relaxed">
                          {receiveImmediately
                            ? "Los productos se marcarán como recibidos y se actualizará el stock automáticamente."
                            : "Los productos quedarán pendientes de recepción y deberán ser recibidos posteriormente."}
                        </p>
                      </div>
                    </div>
                    ...
                    <div
                      className={`flex items-center space-x-2 p-3 rounded-md border ${
                        receiveImmediately ? "bg-green-100 border-green-200" : "bg-yellow-100 border-yellow-200"
                      }`}
                    >
                      {receiveImmediately ? (
                        <>
                          <Truck className="w-4 h-4 text-green-600" />
                          <span className="text-sm font-medium text-green-800">Recepción inmediata activada</span>
                        </>
                      ) : (
                        <>
                          <Clock className="w-4 h-4 text-yellow-600" />
                          <span className="text-sm font-medium text-yellow-800">Recepción posterior</span>
                        </>
                      )}
                    </div>
                  </CardContent>
                  ...
                  <Card className="border-slate-200 bg-slate-50 shadow-sm">
                    <CardContent className="p-4">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <p className="text-sm font-medium text-slate-900">Proveedor</p>
                          <p className="text-sm text-slate-700">{selectedSupplier.nombre}</p>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
            ...
            >
              {loading ? (
                ...
                <>
                  ...
                  {receiveImmediately ? "Confirmar y Recibir" : "Confirmar Compra"}
                </>
              )}
            </Button>
          </div>
