...
const PurchaseProductSelectionModal = ({ isOpen, onClose, onProductSelect, loading: externalLoading = false }) => {
  ...
  const handleCodeSearch = async (searchTerm) => {
    ...
    try {
      console.log("Buscando por código en modal de compras:", searchTerm)

      const searchResult = await searchProducts(searchTerm.trim(), {
        limit: 20,
      })

      console.log("Resultado búsqueda código en modal de compras:", searchResult)
      ...
      if (searchResult.success) {
        ...
        console.log("Coincidencias de código encontradas en modal de compras:", codeMatches.length)
        setCodeSearchResults(codeMatches)
      } else {
        console.error("Error en búsqueda por código en modal de compras:", searchResult.message)
        setCodeSearchResults([])
      }
    } catch (error) {
      console.error("Error en búsqueda por código en modal de compras:", error)
      setCodeSearchResults([])
    }
  }
  ...
  const handleNameSearch = async (searchTerm) => {
    ...
    try {
      console.log("Buscando por nombre en modal de compras:", searchTerm)

      const searchResult = await searchProducts(searchTerm.trim(), {
        limit: 30,
      })

      console.log("Resultado búsqueda nombre en modal de compras:", searchResult)
      ...
      if (searchResult.success) {
        ...
        console.log("Coincidencias de nombre encontradas en modal de compras:", nameMatches.length)
        setNameSearchResults(nameMatches)
      } else {
        console.error("Error en búsqueda por nombre en modal de compras:", searchResult.message)
        setNameSearchResults([])
      }
    } catch (error) {
      console.error("Error en búsqueda por nombre en modal de compras:", error)
      setNameSearchResults([])
    }
  }
  ...
  const renderProduct = (product, searchTerm, searchType) => {
    ...
    return (
      <div
        ...
        className="p-5 hover:bg-slate-50 transition-colors group cursor-pointer border-b border-slate-100 last:border-b-0"
      ...
      >
        <div className="flex items-start justify-between">
          <div className="flex-1 min-w-0">
            ...
            <h4 className="font-semibold text-slate-900 text-lg mb-2 line-clamp-1">
              {searchType === "name" ? (
                <span
                  dangerouslySetInnerHTML={{
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-4 text-sm text-slate-500">
                ...
                <span>
                  Mín: <span className="font-medium">{stockMinimo}</span>
                </span>
                {product.marca && (
              ...
              {product.stock <= stockMinimo && (
                <div className="flex items-center text-orange-600 bg-orange-50 px-2 py-1 rounded-md">
                  ...
                  <span className="text-xs font-medium">Necesita reposición</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="text-right ml-4 flex-shrink-0">
            <div className="text-lg font-bold text-blue-600 mb-1">Costo: {formatCurrency(precioCosto)}</div>
            {product.precio_venta && (
              <div className="text-sm text-slate-500 mb-2">Venta: {formatCurrency(product.precio_venta)}</div>
            )}
            <Button
              onClick={(e) => {
                e.stopPropagation()
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-[100] p-4">
      <div className="bg-white shadow-xl w-full max-w-5xl h-[90vh] flex flex-col overflow-hidden relative z-[101]">
        {/* Header */}
        <div className="flex-shrink-0 flex items-center justify-between p-6 bg-gradient-to-r from-blue-800 to-blue-700">
          <div className="flex items-center space-x-4">
            <div className="w-10 h-10 bg-white/10 rounded-xl flex items-center justify-center backdrop-blur-sm">
              <Package className="w-5 h-5 text-white" />
            </div>
            <div>
              <h2 className="text-xl font-bold text-white">Seleccionar Productos para Compra</h2>
              <p className="text-sm text-blue-100 mt-1">Encuentra productos para agregar a tu orden de compra</p>
            </div>
          </div>
          <Button
            onClick={handleClose}
            variant="ghost"
            size="sm"
            className="text-blue-100 hover:text-white hover:bg-white/10 rounded-lg"
            disabled={externalLoading}
          >
            <X className="w-5 h-5" />
          </Button>
            ...
            {!currentSearch.searchTerm && (
              <div className="text-center py-16">
                ...
                <h3 className="text-xl font-bold text-slate-800 mb-3">
                  {searchType === "code" ? "Escanea o escribe un código" : "Busca productos para comprar"}
                </h3>
                <p className="text-slate-600 mb-6 max-w-md mx-auto">
                  {searchType === "code"
                    ? "Utiliza el lector de códigos de barras o escribe el código manualmente para encontrar productos específicos."
                    : "Escribe el nombre, descripción o marca del producto que necesitas comprar."}
                </p>
